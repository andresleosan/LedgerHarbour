...
- `RATE_LIMIT_MODE=memory`: desarrollo y pruebas. No ofrece proteccion distribuida entre instancias serverless y no es aceptable para produccion.
...
- En `NODE_ENV=production`, cualquier modo distinto de `RATE_LIMIT_MODE=upstash` falla cerrado. El production gate tambien exige ambas variables de Upstash antes de atender requests.
...
Upstash Redis tiene un free tier suficiente para una prueba de bajo volumen, pero sus limites y precios pueden cambiar. Antes de activarlo se deben verificar el plan, los limites, una alerta de billing y el workspace/proyecto correcto. No se reintentan indefinidamente los limites ni se registran IPs, tokens o contraseñas en logs.
