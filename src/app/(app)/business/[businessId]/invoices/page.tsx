    ...
    <main className="operational-page invoice-list-page">
      <div className="page-shell invoice-shell">
        ...
        <section className="invoice-hero" aria-labelledby="invoice-list-title">
          ...
          <nav className="invoice-settings" aria-label="Finance settings"><Link href={`/business/${businessId}/settings/categories?${preservedQuery.toString()}`}>{messages[locale].categories.title}</Link><Link href={`/business/${businessId}/settings/currencies?${preservedQuery.toString()}`}>{messages[locale].currencies.title}</Link></nav>
        </section>
