...
export default async function PlatformAdministratorsPage({ searchParams }: { searchParams?: Promise<{ locale?: string }> }) {
  const identity = await getCurrentIdentity();
  if (!identity) return null;
  const summary = await loadPlatformSummary(identity);
  const locale = (await searchParams)?.locale === "es" ? "es" : "en";
  return <PlatformAdminPanel summary={summary} locale={locale} section="administrators" />;
}
